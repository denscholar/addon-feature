...
import 'core/utils/constant_util.dart';
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        //payment plan
        ChangeNotifierProvider<PaymentPlanModel>(
            create: (_) => locator<PaymentPlanModel>()),
        //business info
        ChangeNotifierProvider<BusinessInfoModel>(
            create: (_) => locator<BusinessInfoModel>()),
        //Team
        ChangeNotifierProvider<TeamModel>(
            create: (_) => locator<TeamModel>()),
        //Business calendar
        ChangeNotifierProvider<BusinessCalendarModel>(
            create: (_) => BusinessCalendarModel()),
        //Service
        ChangeNotifierProvider<ServiceModel>(
            create: (_) => ServiceModel()),
...
class _AuthenticationWrapperState extends State<AuthenticationWrapper> {
  ...
  _initPrefs() async{
    ...
    print('=====================INIT PREFERENCES===========================');
    if (pref != null) {
      // await pref.setBool(DONE_ONBOARDING, true);
      doneOnboarding = pref.getBool(DONE_ONBOARDING);
      print('shared preferences is not null and returnes some values');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: firebaseUser == null
          ? Launcher()
          : doneOnboarding != null && doneOnboarding
              ? HomeView()
                    ...
                    if (snapshot.hasData) {
                      ...
                      if (status == ApiResponseStatus.LOADING)
                        return Column(
                          ...
                          children: [
                            CustomProgressWidget(color: CustomColor.primaryColor,),
                            SizedBox(height: 16),
                            Text('Please wait...'),
                          ],
                        );
